...
Since each element in the population has an equal probability, each of them has a $\frac{1}{N}$ probability.
...
Above is the textbook definition of the mean. There are also other intrepretation of what a mean is. For example, let's define the mean as the number $\alpha$ that is the *closest* to all elements in the population.
...
Alpha <- seq(from = -2, to = 2, by = 0.0001)
sum_abs_diff <- sapply(Alpha, function(x) sum(abs(A-x)))
(alpha <- mean(Alpha[which(sum_abs_diff == min(sum_abs_diff))]))
```

```{r}
all.equal(median(A), alpha, tolerance = 0.001)
```
...
Assume that there is a $\beta$ value that minimizes the sum of all squared differences between $b$ and the elements of the population.

$$
\sum_{i=1}^{N}(A_{i}-\beta)^2
$$

To find the $\beta$ value, we can set its derivative to 0.

$$
\frac{d}{d\beta}(\sum_{i=1}^{N}(A_{i}-\beta)^2) = 0 \\
\frac{d}{d\beta}(\sum_{i=1}^{N}(A_{i}^2 - 2\beta \cdot X_{i} + \beta^2)) = 0 \\
\frac{d}{d\beta}(\sum_{i=1}^{N}A_{i}^2-2\beta \cdot \sum_{i=1}^{N}A_{i}+N\beta^2) = 0 \\
-2\sum_{i=1}^{N}A_{i}+2N\beta = 0 \\
b = \frac{\sum_{i=1}^{N}A_{i}}{N}
$$
...
Beta <- seq(from = -2, to = 2, by = 0.0001)
sum_squared_diff <- sapply(Beta, function(x) sum((A-x)^2))
plot(Beta, sum_squared_diff)
```
...
```{r}
(beta <- Beta[which(sum_squared_diff == min(sum_squared_diff))])
```

We can verify that the $\beta$ value we found is indeed equal to the mean.

```{r}
all.equal(beta, MU, tolerance = 0.001)
```
...
Here $X$ represents a $n$-element sample drawn from $A$.

We can mimic this using a random sample.

```{r}
# Take one 10-element random sample from the population
n <- 10
x <- sample(A, n)
```
